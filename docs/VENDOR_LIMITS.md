...
## 14. 드래프터 TP4 는 두 번째 런타임 생성에서 죽는다 — **원인 미상**

> **2026-09-02 정정.** 이 항목은 원래 "정적 주소 KV 캐시를 공유하는 TP 런타임은
> 두 개 만들 수 없다" 라는 제목이었습니다. **그 일반화는 틀렸습니다** — 타깃이
> 정확히 그 구성으로 돌고 있습니다 (아래 반례). 관측 사실만 남기고 원인은
> 미상으로 되돌립니다.
...
남은 차이는 **누가 그래프를 만들었는가** 하나입니다 — 타깃은 optimum-rbln 의
컴파일 경로를 타고, 드래프터는 `draft_two_rr.py` 에 직접 짠 `PAGED` 커스텀 op
그래프입니다. 캐시를 카드별로 어떻게 쪼갤지 정하는 지점이 다를 것으로 의심되지만,
그 결정을 들여다보거나 지정할 인터페이스가 없어 확인하지 못했습니다.
...
```
block   TP4 로 4.63 -> 1.43 ms 가능하지만 쓸 수 없다      -3.2 ms 상실
