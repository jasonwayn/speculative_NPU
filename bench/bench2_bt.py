...
LID=draft.target_layer_ids; BT=torch.arange(int(getattr(m.rbln_config,"kvcache_num_blocks",1) or 1),dtype=torch.int16)  # flash_attn 은 블록이 여러 개
...
for C in BUCKETS:
    ...
    BK[C]=dict(rt=cm.create_runtime(device=DEV), TH=torch.zeros(1,C,NT*H,dtype=DT),
               PO=torch.zeros(1,C+B,dtype=torch.long), MK=torch.zeros(1,1,B,C+B,dtype=DT))
...
print("blocks="+str(BT.numel())+" compiled "+str(round(time.time()-t0,1))+"s",flush=True)
if os.environ.get("SERIAL_LOAD","1")=="1":
...
def run(dsname,warm=False):
    ...
    for ex in ds:
        ...
        while VL<P+mx:
            ...
            pn=k-L
            b["TH"][:,pn:]=th; b["PO"][:,pn:]=torch.arange(L+B)
