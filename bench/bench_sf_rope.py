...
for _ch in dict.fromkeys((CH_V, CH_P)):
    ...
    _gcfg = _copy.deepcopy(_mcfg); _gcfg.num_hidden_layers = 5 if _ch == CH_V else 4
...
def ainfo(A):
    return [("th_new", [1, A, NT * H], DTS), ("cos_ctx", [1, A, HD], DTS), ("sin_ctx", [1, A, HD], DTS),
            ("seq_ctx", [1, 1], "int32"),
            ("block_tables", [KV_NUM_BLOCKS], "int16")] + cinfo


binfo = [("noise_emb", [1, B, H], DTS), ("cos_blk", [1, B, HD], DTS), ("sin_blk", [1, B, HD], DTS),
         ("seq_blk", [1, 1], "int32"),
...
t0 = time.time()
cm_a = rebel.compile_from_torch(AppendR(draft, KV_BLOCK_SIZE).eval(), input_info=ainfo(APPEND_WIDTH),
...
if NONCAUSAL_DRAFT:
    ...
    _block_module = (_BlockTrace(draft, KV_BLOCK_SIZE).eval()
                     if DRAFT_TRACE else BlockR(draft, KV_BLOCK_SIZE).eval())
...
def run(warm=False):
    ...
    def tpre(seg, off):
        ...
        T["verify"] += time.time() - s; hs = r.hidden_states[:5]
...
run(warm=True)
with Power(DEV_TARGET) as p0: time.sleep(5)
IDLE = p0.stats().get("P_mean", 0.0)
with Power(DEV_TARGET) as pw:
