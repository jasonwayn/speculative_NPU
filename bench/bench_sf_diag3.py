...
CMR_STUB = int(os.environ.get("CMR_STUB", "0"))        # 스코어러 제거, 최근 BUDGET 개 선택
CMR_NOAPPLY = int(os.environ.get("CMR_NOAPPLY", "0"))  # 캐시 재기록 생략
KST_PRE = int(os.environ.get("KST_PRE", "0"))          # 키 저장소 사전할당 (라운드마다 cat 안 함)
...
rt_a = rebel.Runtime(cm_a, tensor_type="pt", device=DEV_DRAFT)
rt_b = rebel.Runtime(cm_b, tensor_type="pt", device=DEV_DRAFT)
rrt = rebel.Runtime(cm_r, tensor_type="pt", device=DEV_DRAFT) if cm_r is not None else None
lrt = lcm.create_runtime(device=DEV_DRAFT)
...
def run(warm=False):
    ...
    def tpre(seg, off):
        ...
        _d = time.time() - s
        T["verify"] += _d; _lastv[0] = _d; hs = r.hidden_states[:6]   # 레이어 35 포함
    ...
    for si in range(1 if warm else NSAMP):
        ...
        if CMR:
            ...
            if CMR_STUB:
                _lo = max(0, TL - BUDGET)
                sel = torch.arange(_lo, TL, dtype=torch.long)
            else:
        ...
        while VL < P + (WARMNEW if warm else MAXNEW):
            ...
            acc.append(a + 1)
            VT.append((_lastv[0], _rflag[0])); _rflag[0] = 0
        ...
        ntok += VL - P
    return acc, ntok, T, nret, kept, VT
...
with Power(_PWR_DEVS) as pw:
    acc, ntok, T, nret, kept, VT = run()
st = pw.stats(idle=IDLE)
n = max(len(acc), 1)
import resource as _res
_va = [v for v, f in VT if f]; _vb = [v for v, f in VT if not f]
_srt = sorted(v for v, _ in VT)
def _q(p):
    return round(1000 * _srt[min(int(len(_srt) * p), len(_srt) - 1)], 1) if _srt else 0.0
_vst = dict(v_postret_ms=round(1000 * sum(_va) / max(len(_va), 1), 1), v_postret_n=len(_va),
            v_other_ms=round(1000 * sum(_vb) / max(len(_vb), 1), 1), v_other_n=len(_vb),
            v_p10_ms=_q(0.10), v_p50_ms=_q(0.50), v_p90_ms=_q(0.90),
            v_max_ms=round(1000 * _srt[-1], 1) if _srt else 0.0,
            stub=CMR_STUB, noapply=CMR_NOAPPLY, kstpre=KST_PRE,
            nthr=torch.get_num_threads(), omp=os.environ.get("OMP_WAIT_POLICY", "-"),
            nthr_env=os.environ.get("NTHREADS", "-"),
            os_threads=int(open("/proc/self/status").read().split("Threads:")[1].split()[0]),
            load1=round(float(open("/proc/loadavg").read().split()[0]), 2),

            rss_gb=round(_res.getrusage(_res.RUSAGE_SELF).ru_maxrss / 1e6, 2))
r = dict(mode="stateful", dset=DSET, cmr=CMR, inlen=INLEN,
         target_device=TARGET_DEVICES, target_tp=TARGET_TP, draft_device=DEV_DRAFT,
         ...
         retrievals=nret, kept_mean=round(kept / max(nret, 1), 1), **_vst,
