...
for _ch in (CH_V, CH_P):
    _cm = _rebel.RBLNCompiledModel(
        "/home/work/npu_work/dflash_work/slim_17_256/prefill_%d.rbln" % _ch)
...
def run(warm=False):
    ...
    def twh(ids):
        s = time.time(); L = ids.shape[1]; pad = 1 if L % CH_P == 0 else 0
        i2 = torch.cat([ids, ids[:, -1:]], dim=1) if pad else ids
        hs = DUAL[CH_P].prefill_forward(i2, cache_position=torch.arange(i2.shape[1], dtype=torch.int32).unsqueeze(0), attention_mask=torch.ones(i2.shape[1], dtype=torch.int64), batch_idx=0, block_tables=BT, is_external_block_tables=False).hidden_states
...
n = max(len(acc), 1)
r = dict(mode="stateful", dset=DSET, cmr=CMR, inlen=INLEN, samples=NSAMP, tau=round(sum(acc) / n, 3),
