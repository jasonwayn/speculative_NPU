"""bench_sf_fused_diag.py -> bench_sf_rr.py : 드래프터 RoPE 를 호스트 범위축소 + 그래프 cos/sin 으로 교체.
...
dst = "/home/work/npu_work/dflash_work/bench_sf_rr.py"
...
t = t.replace("Append(draft, KV_BLOCK_SIZE)", "AppendRR(draft, KV_BLOCK_SIZE)")
n["cls_b"] = t.count("Block(draft, KV_BLOCK_SIZE)")
t = t.replace("Block(draft, KV_BLOCK_SIZE)", "BlockRR(draft, KV_BLOCK_SIZE)")

# ② 입력 서명
n["ainfo"] = t.count('("pos_ctx", [1, A], "int32")')
t = t.replace('("pos_ctx", [1, A], "int32")',
              '("angle_ctx", [1, A, HD // 2], DTS)')
