"""비정렬 오프셋 prefix caching 이 실제로 되는가 (가드 제거).

가드가 지키는 코드는 전부 `if use_attention_mask:` 아래인데 우리 모델은 False 다.
zero-arg super() 는 클래스 밖에서 못 쓰므로 super(_Base, self) 로 바꿔 모듈 레벨에 정의한다.
"""
import sys, inspect, textwrap, torch
import optimum.rbln.transformers.models.decoderonly.decoderonly_runtime_utils as RU

M = RU.RBLNRuntimeModel
lines = inspect.getsource(M.prefill_forward).split("\n")
...
ns = dict(RU.__dict__)
ns["_Base"] = M
exec(compile(body, "<patched>", "exec"), ns)
...
try:
    ...
    ok = False
    print("UNALIGNED_FAIL %s: %s" % (type(e).__name__, str(e)[:220]), flush=True)
...
print("UNALIGN3_DONE", flush=True)
